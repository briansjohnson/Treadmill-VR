...
namespace VRTK
{
    ...
    public class VRTK_MoveInPlace : MonoBehaviour
    {
        ...
        /*
        public enum StrafingMethod
        {
            StrafeControllerRotation, 
            ButtonMovement
        }
        */
        ...
        public VRTK_ControllerEvents.ButtonAlias engageButton = VRTK_ControllerEvents.ButtonAlias.GripPress;
   
        //[Tooltip("Select which button to engage Strafe.")]
        //public VRTK_ControllerEvents.ButtonAlias engageStrafe = VRTK_ControllerEvents.ButtonAlias.TouchpadPress;
        ...
        //[Tooltip("Select Strafing method")]
        //public StrafingMethod strafeMethod = StrafingMethod.ButtonMovement;
        ...
        //protected VRTK_ControllerReference engagedControllerStrafe;
        ...
        //protected VRTK_ControllerEvents.ButtonAlias previousStrafeButton;
        ...
        //protected float strafeCurrentSpeed;

        protected Vector3 direction;
        protected Vector3 previousDirection;
        // The current strafe direction the player is moving. If Strafe is not active, it will be set to Vector.zero.
        //protected Vector3 strafeDirection;
        //protected Vector3 strafePreviousDirection;

        // True if Move In Place is currently engaged.
        protected bool active;

        // True if Strafe Movement is active.
        //protected bool strafeActive;
        ...
        protected virtual void SetControlOptions(ControlOptions givenControlOptions)
        {
        ...
        /// Will return the strafe direction the player is moving.

        /*
        protected virtual Vector3 GetMovementStrafeDirection()
        {
        ...
        */
        ...
        /*
        protected virtual float GetStrafeSpeed()
        {
            return strafeCurrentSpeed;
        }
        */
        ...
        protected virtual void OnEnable()
        {
            ...
            //strafePreviousDirection = Vector3.zero;
            ///
            averagePeriod = 60;
            currentSpeed = 0f;
            active = false;
            ...
            ///
            //strafeActive = false;
            //previousStrafeButton = engageStrafe;
            ///
            ///
        ...
        //movement activated.......need to change for strafe
        ...
        protected virtual void FixedUpdate()
        {
            ...
            else
            {
                ...
                direction = Vector3.zero;
                previousDirection = Vector3.zero;
                //StrafeMovementActivated();
            }
            /*
            if (StrafeMovementActivated() && !currentlyFalling)
            {
                float strafeSpeed = Mathf.Clamp(((speedScale * 350) * (CalculateListAverage() / trackedObjects.Count)), 0f, maxSpeed);

                strafePreviousDirection = strafeDirection;
                strafeDirection = SetStrafeDirection();

                // Update our current speed.
                strafeCurrentSpeed = strafeSpeed;
                //print(strafeCurrentSpeed);
            }
            else if (strafeCurrentSpeed > 0f)
            {
                strafeCurrentSpeed -= (currentlyFalling ? fallingDeceleration : deceleration);
            }
            else
            {
                MovementActivated();
                strafeCurrentSpeed = 0f;
                ///
                strafePreviousDirection = Vector3.zero;
                strafeDirection = Vector3.zero;
            }
            */
            ...
            /// Add StrafePlayArea(strafeDirection, currentSpeed); //possibly currentStrafeSpeed
            //StrafePlayArea(strafeDirection, strafeCurrentSpeed);
        }

        ///Bug is coming from here! changing to button movement for strafe fixed the bug
        
        /// <summary>
        /// possibly change to if else statement
        /// </summary>
        /// <returns></returns>
        ...
        /*
        protected virtual bool StrafeMovementActivated()
        {
            return (strafeActive || engageStrafe == VRTK_ControllerEvents.ButtonAlias.Undefined);
        }
        */
        ...
        {
            if (controllerState != previousState || engageButton != previousEngageButton) //&& engageStrafe == VRTK_ControllerEvents.ButtonAlias.Undefined)
            {
            ...
            /*
            else if (controllerState != previousState || engageStrafe != previousStrafeButton && engageButton == VRTK_ControllerEvents.ButtonAlias.Undefined)
            {
                SetControllerListeners(controller, controllerState, ref subscribedState);
            }
            */
            
            else if (controllerState != previousState || engageButton != previousEngageButton) //&& (controllerState != previousState || engageStrafe != previousEngageButton))
            {
                SetControllerListeners(controller, controllerState, ref subscribedState);
            }
        ...
        protected virtual float CalculateListAverage()
        {
            ...
            for (int i = 0; i < trackedObjects.Count; i++)
            {
                ...
                //Debug.Log("Logging Delta:" + deltaYPostion);
                
                ///
                SpinCylinder.transform.Rotate(0, 0, -(deltaYPostion) * 2f);
                ///
        ...
        /*
        protected virtual Vector3 SetStrafeDirection()
        {
            switch (strafeMethod)
            {
                case StrafingMethod.StrafeControllerRotation:
                    Debug.Log("hey" + SetStrafeDirection());
                    return StrafeCalculateControllerRotationDirection((engagedControllerStrafe != null ? engagedControllerStrafe.scriptAlias.transform.rotation : Quaternion.identity) * Vector3.forward);
        ...
        */
        ...
        /// <summary>
        /// Calculate Strafe direction...needs fixing
        /// </summary>
        /// <param name="strafeCalculatedControllerDirection"></param>
        /// <returns></returns>
        /*
        protected virtual Vector3 StrafeCalculateControllerRotationDirection(Vector3 strafeCalculatedControllerDirection)
        {
            return (Vector3.Angle(strafePreviousDirection, strafeCalculatedControllerDirection) <= 90f ? strafeCalculatedControllerDirection : strafePreviousDirection);
        }
        */
        ...
        //update movement variable to strafeMovement
        /*
        protected virtual void StrafePlayArea(Vector3 strafeDirection, float moveSpeed)
        {
            Vector3 movement = (strafeDirection * moveSpeed) * Time.fixedDeltaTime;
            Vector3 finalPosition = new Vector3(movement.x + playArea.position.x, playArea.position.y, movement.z + playArea.position.z);
            if (playArea != null && CanMove(bodyPhysics, playArea.position, finalPosition))
            {
                playArea.position = finalPosition;
        ...
        */
        ...
        /*
        protected virtual void EngageStrafePressed(object sender, ControllerInteractionEventArgs e)
        {
            engagedControllerStrafe = e.controllerReference;
            strafeActive = true;
        }

        protected virtual void EngageStrafeReleased(object sender, ControllerInteractionEventArgs e)
        {
            //If the button is released, clear all the lists.
            for (int i = 0; i < trackedObjects.Count; i++)
            {
        ...
        */
        ...
        protected virtual void ToggleControllerListeners(GameObject controller, bool toggle, ref bool subscribed)
        {
            ...
            if (controllerEvent != null)
            {
                ...
                if (engageButton != previousEngageButton && subscribed) // && engageStrafe != previousStrafeButton)
                {
                    ...
                    //controllerEvent.UnsubscribeToButtonAliasEvent(previousStrafeButton, true, EngageStrafePressed);
                    //controllerEvent.UnsubscribeToButtonAliasEvent(previousStrafeButton, false, EngageStrafeReleased);
